...
class SaleReportGenerator(models.Model):
    ...
    report_type = fields.Selection(
        [
            ...
            ("all_salesperson_report", "All Sales Person Report"),
            ("own_salesperson_report", "Own Sales Person Report"),
        ],
    ...
    def sale_report(self, option):
        ...
        report_values = self.env["sales.report"].search([("id", "=", option[0])])
        data = {
    ...
    def get_filter(self, option):
        ...
        elif data.get("report_type") == "own_salesperson_report":
            filters["report_type"] = "Own Sales Person Report"
        else:
            filters["report_type"] = "report_by_order"

        if data.get("report_type") == "own_salesperson_report":
            filters["own_salesperson_report"] = True
    ...
    def get_filter_data(self, option):
        ...
        if r.report_type == "own_salesperson_report":
            filter_dict["own_salesperson_report"] = True
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get("report_type") == "report_by_order":
            ...
            query += (
                "group by so.user_id,res_users.partner_id,"
            ...
            query = """
                       SELECT  so.id,so.name as number,so.date_order,res_partner.name as customer,
                            rc.name as company,COALESCE(jsonb_extract_path_text(product_template.name::jsonb, 'en_US')) as product,
            ...
            # print(report_by_order_details)
            report_sub_lines.append(report_by_order_details)

        elif data.get("report_type") == "report_by_product":
            query = """
                        SELECT  so.id,so.date_order,COALESCE(jsonb_extract_path_text(product_template.name::jsonb, 'en_US')) as product,
            ...
            term = "Where "
            if data.get("date_from"):
                query += "Where sale_order.date_order >= '%s' " % data.get("date_from")
                term = "AND "
            if data.get("date_to"):
                query += term + "sale_order.date_order <= '%s' " % data.get("date_to")
            ...
            query = (
                ...
                SELECT res_users.partner_id, res_partner.name as salesperson, 
            ...
            if data.get("date_from"):
                query += term + "so.date_order >= '%s' " % data.get("date_from")
            ...
            query += "GROUP BY res_users.partner_id, res_partner.name"

            self._cr.execute(query)

            own_salesperson_report = self._cr.dictfetchall()

            report_sub_lines.append(own_salesperson_report)
    ...
    def _get_report_total_value(self, data, report):
        ...
        if data.get("report_type") == "report_by_order":
            ...
            # Add your custom logic here for "own_salesperson_report"
            # For example:
            self._cr.execute(
                """
                    SELECT res_users.partner_id, res_partner.name as salesperson,
                        count(so.id) as order, sum(sale_order_line.price_subtotal) as amount
                    FROM sale_order AS so
                    INNER JOIN res_users ON so.user_id = res_users.id
                    INNER JOIN res_partner ON res_users.partner_id = res_partner.id
                    INNER JOIN sale_order_line ON so.id = sale_order_line.order_id
            ...
            own_salesperson_report = self._cr.dictfetchall()
            report_main_lines.append(own_salesperson_report)
    ...
    def _get_report_values(self, data):
        ...
        if data["report_type"] == "report_by_order_detail":
        ...
        elif data["report_type"] == "own_salesperson_report":
            # Custom logic for "own_salesperson_report"
            query = (
                """
                    SELECT res_users.partner_id, res_partner.name as salesperson, 
                        sum(sale_order_line.product_uom_qty) as qty,
            ...
            term = "AND "
            if date_from:
                query += term + "so.date_order >= '%s' " % date_from
                term = "AND "
            if date_to:
                query += term + "so.date_order <= '%s' " % date_to
            query += "GROUP BY res_users.partner_id, res_partner.name"
            self._cr.execute(query)
            own_salesperson_report = self._cr.dictfetchall()
            report_res_total = own_salesperson_report
            report_res = own_salesperson_report
            return {
                "doc_ids": self.ids,
                "docs": docs,
                "SALE": report_res,
    ...
    def get_sale_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range("A2:H3", "Sales Report", head)
        ...
        if filters.get("report_type") == "report_by_order":
            sheet.merge_range(
                "B5:D5", "Report Type: " + filters.get("report_type"), txt_l
            )
        ...
        if filters.get("report_type") == "report_by_order_detail":
            ...
            for rec_data in report_data_main:
                ...
                # first_key = next(iter(product))
                # # Get the value of the first key
                # first_value = product[first_key]
        ...
        if filters.get("report_type") == "own_salesperson_report":
            sheet.merge_range(
                "B5:D5", "Report Type: " + filters.get("report_type"), txt_l
            )

            sheet.write("A7", "Sales Person", heading)
            sheet.write("B7", "Total Qty", heading)
            sheet.write("C7", "Total Amount", heading)
            ...
            for rec_data in report_data_main:
                ...
                two_lst = []
                row += 1
                sheet.write(row, col, rec_data["salesperson"], txt_l)
                sheet.write(row, col + 1, rec_data["qty"], txt_l)
                sheet.write(row, col + 2, rec_data["amount"], txt_l)
